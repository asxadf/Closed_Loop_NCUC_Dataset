%% --------------------------------Hello-------------------------------- %%
% This code details the UC model in our paper: Feature-driven Economic
% Improvement for Network-Constrained Unit Commitment: A Closed-Loop
% Predict-and-Optimize Framework.
%
% Please let me know if you have concerns about this code.
% It is my pleasure to discuss/explain with you.
%
% My academic email: xchen130@stevens.edu
% My personal email: chenxianbang@hotmail.com
%
% Please cite our paper if you use this code in your research:
%
% Xianbang Chen, Yafei Yang, Yikui Liu, and Lei Wu. "Feature-driven Economic
% Improvement for Network-Constrained Unit Commitment: A Closed-Loop
% Predict-and-Optimize Framework," IEEE Transaction on Power Systems,
% vol. 37, no. 4, pp. 3104-3118, July 2022, doi: 10.1109/TPWRS.2021.3128485.
%% --------------------------------Hello-------------------------------- %%
%
clc;
clear;
close all;
%% -------------------------------Setting------------------------------- %%
Dispatch_day = 1;
Day_1st      = Dispatch_day;
Day_end      = Dispatch_day;
Scaler_load  = 0.22;
Scaler_SPG   = 0.39;
Scaler_WPG   = 0.39;
R_for_load   = 0.10; % Reserve for load
R_for_RES    = 0.05; % Reserve for RES
R_h_level    = 0.20; % The least-level spinning reserve of total reserve
Method_flag  = 'OPO';
%% -------------------------------Loading------------------------------- %%
[Number_gen,...
 Number_branch,...
 Number_bus,...
 Number_city,...
 Number_day,...
 Number_hour,...
 Number_RES,...
 Number_seg,...
 Number_point,...
 Point_Gen,...
 Point_price,...
 Data_Gen_capacity,...
 Data_Gen_price,...
 Data_branch,...
 Data_load_country,...
 Data_load_city,...
 Data_RES_DAF,...
 Data_RES_RUM,...
 Data_feature,...
 PTDF_Gen,...
 PTDF_city,...
 PTDF_RES,...
 L_s_price] = CPO_Database_Belgium_bus24(Day_1st,...
                                         Day_end,...
                                         Scaler_load,...
                                         Scaler_SPG,...
                                         Scaler_WPG,...
                                         Method_flag);
%% -------------------------------Decision------------------------------ %%
% UC decision
Decision_I          = binvar(Number_gen, Number_hour);
Decision_I_SU       = binvar(Number_gen, Number_hour);
Decision_I_SD       = binvar(Number_gen, Number_hour);
Decision_P          = sdpvar(Number_gen, Number_hour);
Decision_P_cost     = sdpvar(Number_gen, Number_hour);
Decision_delta      = sdpvar(Number_gen*Number_point, Number_hour);
Decision_R_h        = sdpvar(Number_gen, Number_hour);
Decision_R_c        = sdpvar(Number_gen, Number_hour);
Decision_R_all_req  = sdpvar(Number_hour, 1);
Decision_R_load_req = sdpvar(Number_hour, 1);
Decision_R_RES_req  = sdpvar(Number_hour, 1);
% ED decision
Decision_L_s = sdpvar(Number_hour, Number_city);
Decision_L_r = sdpvar(Number_hour, Number_city);
Decision_W_s = sdpvar(Number_hour, Number_RES);
Decision_W_r = sdpvar(Number_hour, Number_RES);
%% -------------------------Constraints: general------------------------ %%
CC_General_01 = [];
CC_General_02 = [];
CC_General_03 = [];
CC_General_04 = [];
CC_General_05 = [];
CC_General_06 = [];
CC_General_07 = [];
CC_General_08 = [];
CC_General_09 = [];
% General_01: Non-negative
CC_General_01 = CC_General_01...
              + [Decision_R_h        >= 0]...
              + [Decision_R_c        >= 0]...
              + [Decision_R_all_req  >= 0]...
              + [Decision_R_load_req >= 0]...
              + [Decision_R_RES_req  >= 0]...
              + [Decision_L_s        >= 0]...
              + [Decision_L_s        <= 0]...
              + [Decision_L_r        >= 0]...
              + [Decision_W_s        >= 0]...
              + [Decision_W_r        >= 0];
% General_02: Locally ideal formulation of operation cost
for t = 1:Number_hour
    for i = 1:Number_gen
        % Generation cost
        CC_General_02 = CC_General_02...
        + [   Decision_P_cost(i,t)...
           == Point_price(i,:)...
              *Decision_delta((i-1)*Number_point+1:i*Number_point,t)];
    end
end
for t = 1:Number_hour
    for i = 1:Number_gen
        % Commitment decision
        CC_General_02 = CC_General_02...        
        + [   Decision_I(i,t)...
           == sum(Decision_delta((i-1)*Number_point+1:i*Number_point,t))];
    end
end
for t = 1:Number_hour
    for i = 1:Number_gen
        % Generation decision
        CC_General_02 = CC_General_02...
        + [   Decision_P(i,t)...
           == Point_Gen(i,:)*Decision_delta((i-1)*Number_point+1:i*Number_point,t)];
    end
end
CC_General_02 = CC_General_02 + [Decision_delta >= 0];
CC_General_02 = CC_General_02 + [Decision_delta <= 1];
%
% General_03: Generation limit
for t = 1:Number_hour
    CC_General_03 = CC_General_03...
    + [Decision_P(:,t) - Decision_R_h(:,t) >= Decision_I(:,t).*Data_Gen_capacity(:,3)];
end
for t = 1:Number_hour
    CC_General_03 = CC_General_03...
    + [Decision_P(:,t) + Decision_R_h(:,t) <= Decision_I(:,t).*Data_Gen_capacity(:,4)];
end
%
% General_04: Logical relationship
CC_General_04 = CC_General_04...
+ [Decision_I_SU(:,1) - Decision_I_SD(:,1) == Decision_I(:,1)];
for t = 2:Number_hour
    CC_General_04 = CC_General_04...
    + [Decision_I_SU(:,t) - Decision_I_SD(:,t) == Decision_I(:,t) - Decision_I(:,t-1)];
end
%
% General_05: Min on/off
for i = 1:Number_gen
    for t = Data_Gen_capacity(i,5):Number_hour
        CC_General_05 = CC_General_05...
        + [sum(Decision_I_SU(i,t-Data_Gen_capacity(i,5)+1:t)) <= Decision_I(i,t)];
    end
end
for i = 1:Number_gen
    for t = Data_Gen_capacity(i,6):Number_hour
        CC_General_05 = CC_General_05...
        + [sum(Decision_I_SD(i,t-Data_Gen_capacity(i,6)+1:t)) <= 1 - Decision_I(i,t)];
    end
end
%
% General_06: Ramping limit
for t = 2:Number_hour
    CC_General_06 = CC_General_06...
    + [   Decision_P(:,t) - Decision_P(:,t-1)...
       <= Data_Gen_capacity(:,7).*Decision_I(:,t-1)...
        + Data_Gen_capacity(:,9).*(Decision_I(:,t) - Decision_I(:,t-1))...
        + Data_Gen_capacity(:,4).*(1 - Decision_I(:,t))];
    CC_General_06 = CC_General_06...
    + [   Decision_P(:,t-1) - Decision_P(:,t)...
       <= Data_Gen_capacity(:,8).*Decision_I(:,t)...
        + Data_Gen_capacity(:,10).*(Decision_I(:,t-1) - Decision_I(:,t))...
        + Data_Gen_capacity(:,4).*(1 - Decision_I(:,t-1))];
end
%
% General_07: Power balance
for t = 1:Number_hour
    CC_General_07 = CC_General_07...
    + [sum(Decision_P(:,t)) + sum(Decision_W_r(t,:)) == sum(Decision_L_r(t,:))];
end
%
% General_08: Line capacity limit
for t = 1:Number_hour
    CC_General_08 = CC_General_08...
    + [- Data_branch(:,4)...
       <= PTDF_Gen*Decision_P(:,t) + PTDF_RES*Decision_W_r(t,:)' - PTDF_city*Decision_L_r(t,:)'];
end
for t = 1:Number_hour
    CC_General_08 = CC_General_08...
    + [PTDF_Gen*Decision_P(:,t) + PTDF_RES*Decision_W_r(t,:)' - PTDF_city*Decision_L_r(t,:)'...
       <= Data_branch(:,4)];
end
%
% General_09: Reserve requirement
for t = 1:Number_hour
    CC_General_09 = CC_General_09...
                  + [sum(Decision_R_h(:,t)) >= R_h_level*Decision_R_all_req(t)]...
                  + [Decision_R_h(:,t) <= Data_Gen_capacity(:,11).*Decision_I(:,t)]...
                  + [Decision_R_c(:,t) == Decision_R_h(:,t) + (1 - Decision_I(:,t)).*Data_Gen_capacity(:,12)]...
                  + [Decision_R_all_req(t) <= sum(Decision_R_h(:,t)) + sum(Decision_R_c(:,t))];
end
%% -------------------------Constraints: special------------------------ %%
CC_Special = [];
% CC_Special_01: Load shedding limit
Load_RUM      = Data_load_city{24*(Day_1st-1)+1:24*Day_end, :};
Country_Load  = sum(Load_RUM,2);
CC_Special    = CC_Special + [Decision_L_s(:) + Decision_L_r(:) == Load_RUM(:)];
% CC_Special_02: RES curtailment limit
RES_DAF     = Data_RES_DAF{(24*(Day_1st-1)+1:24*Day_end), :};
Country_RES = sum(RES_DAF,2);
CC_Special  = CC_Special + [Decision_W_s(:) + Decision_W_r(:) == RES_DAF(:)];
% CC_Special_03: Provided reseve
CC_Special = CC_Special...
           + [Decision_R_load_req == R_for_load*Country_Load]...
           + [Decision_R_RES_req  == R_for_RES*Country_RES]...
           + [Decision_R_load_req + Decision_R_RES_req == Decision_R_all_req];
%
%% ---------------------------Constraints: all-------------------------- %%
CC_General = CC_General_01 + CC_General_02 + CC_General_03 + CC_General_04...
           + CC_General_05 + CC_General_06 + CC_General_07 + CC_General_08...
           + CC_General_09;
CC = CC_General + CC_Special;
%
%% ------------------------------Objective------------------------------ %%
Cost_SU  = Data_Gen_price(:,5)'*sum(Decision_I_SU,2);
Cost_SD  = Data_Gen_price(:,6)'*sum(Decision_I_SD,2);
Cost_P   = sum(Decision_P_cost(:));
Cost_L_s = sum(L_s_price*Decision_L_s(:));
Cost_UC  = Cost_SU + Cost_SD + Cost_P + Cost_L_s...
         + 0.0001*sum(Decision_R_h(:)) + 0.0001*sum(Decision_R_c(:));
%% -------------------------------Solve it------------------------------ %%
ops = sdpsettings('solver', 'gurobi');
obj = optimize(CC, Cost_UC, ops);
UC_TIME = obj.solvertime;
%% -------------------------------Value it------------------------------ %%
Decision_I          = round(value(Decision_I));
Decision_I_SU       = round(value(Decision_I_SU));
Decision_I_SD       = round(value(Decision_I_SD));
Decision_R_h        = round(value(Decision_R_h));
Decision_R_c        = round(value(Decision_R_c));
Decision_R_all_req  = value(Decision_R_all_req);
Decision_R_load_req = value(Decision_R_load_req);
Decision_R_RES_req  = value(Decision_R_RES_req);

Decision_P      = round(value(Decision_P),5);
Decision_P_cost = value(Decision_P_cost);
Decision_delta  = value(Decision_delta);
Decision_L_s    = value(Decision_L_s);
Decision_L_r    = value(Decision_L_r);
Decision_W_s    = value(Decision_W_s);
Decision_W_r    = value(Decision_W_r);

Cost_SU  = value(Cost_SU);
Cost_SD  = value(Cost_SD);
Cost_P   = value(Cost_P);
Cost_L_s = value(Cost_L_s);
Cost_UC  = value(Cost_UC);
